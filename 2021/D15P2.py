...
grid_of_grids = [[None]*5 for _ in range(5)]
for i in range(5):
    for j in range(5):
        grid_of_grids[i][j] = array + (i+j)
        grid_of_grids[i][j][grid_of_grids[i][j]>9]+=1
        grid_of_grids[i][j][grid_of_grids[i][j]>9]%=10

big_array = np.concatenate([np.concatenate(row, axis = 1) for row in grid_of_grids])
array = big_array
